...
| 工具名 | 描述 | 参数 |
|--------|------|------|
| `stock_zh_a_spot_em` | 沪深京A股实时行情 | `limit` (可选) - 数据条数限制 |
| `stock_sh_a_spot_em` | 沪A股实时行情 | `limit` (可选) |
| `stock_sz_a_spot_em` | 深A股实时行情 | `limit` (可选) |
| `stock_cy_a_spot_em` | 创业板实时行情 | `limit` (可选) |
| `stock_kc_a_spot_em` | 科创板实时行情 | `limit` (可选) |
| `stock_zh_b_spot_em` | B股实时行情 | `limit` (可选) |
| `stock_us_spot_em` | 美股实时行情 | `limit` (可选) |
...
| 工具名 | 描述 | 参数 |
|--------|------|------|
| `stock_individual_info_em` | 个股基本信息 | `symbol` (必需) |

### 市场分析类

| 工具名 | 描述 | 参数 |
|--------|------|------|
| `stock_zh_ah_spot_em` | A+H股比价数据 | `limit` (可选) |
| `stock_zh_valuation_comparison_em` | 同行估值比较 | `symbol` (必需) |
...
本服务器基于 [AkShare](https://github.com/akfamily/akshare) 库，数据来源于：
- 东方财富网
- 上海证券交易所
- 深圳证券交易所
- 新浪财经
...
1. 在 `src/index.ts` 中的 `getToolDefinitions` 方法中添加工具定义
2. Python服务会自动调用对应的akshare函数
3. 在 `test/mcp-test.js` 中添加相应的测试用例
