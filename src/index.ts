...
class MarketMCPServer {
  ...
  private async handleGetStockHistory(args: any) {
    ...
    if (result.success && result.data.length > 0) {
      ...
      if (start_date) formattedData += `- 开始日期: ${start_date}\n`;
      if (end_date) formattedData += `- 结束日期: ${end_date}\n`;
      ...
      if (result.success && result.data) {
        ...
        if (result.data.sectorStats) {
          ...
          for (const [industry, stats] of Object.entries(result.data.sectorStats)) {
            const sectorStats = stats as any;
  ...
  private async handleCheckServiceStatus(args: any) {
  ...
  private async getEastMoneyStockData(codes: string[]): Promise<{ success: boolean; data: any[]; error?: string }> {
    try {
  ...
  private formatBasicStockData(stocks: any[], source: string): string {
