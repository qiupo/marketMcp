...
export class IPO3ServiceV2 {
   ...
   * 通用HTTP请求方法
   */
  private async request(url: string, timeout: number = 15000): Promise<string> {
    try {
   ...
   * 提取表格数据
   */
  private extractTableData($: cheerio.CheerioAPI, selector: string): Array<Record<string, string>> {
   ...
   * 获取公司详细信息（核心功能）
   */
  async getCompanyInfo(stockCode: string, englishKey: boolean = false): Promise<CompanyInfo> {
   ...
   */
  private parseComplexSections($: cheerio.CheerioAPI, data: any): void {
    // 解析各个标签页
    ...
    titles.each((index, title) => {
      ...
      if (key === '公司简介' || key === '主营业务' || key === '经营范围') {
        ...
        data.equityStructure = items.map((item: any) => ({
   ...
   * 解析股本结构
   */
  private parseEquityStructureFromHTML(html: string): any[] {
    const $ = cheerio.load(html);
    const items = this.extractTableData($, 'table');
   ...
   * 解析高管介绍
   */
  private parseSeniorManagementFromHTML(html: string): any[] {
    const $ = cheerio.load(html);
   ...
   * 解析新闻列表
   */
  private parseNewsListFromHTML(html: string): any[] {
    const $ = cheerio.load(html);
    const newsItems: any[] = [];
   ...
   * 获取利润表数据
   */
  async getIncomeStatementList(stockCode: string, dateType: string = '年报', englishKey: boolean = false): Promise<IncomeStatement[]> {
    ...
    if (englishKey) {
      return tableData.map(item => this.convertKeys(this.keyMappings.companyInfo, item) as IncomeStatement);
    }

    return tableData as IncomeStatement[];
  }

  /**
   * 获取资产负债表数据
   */
  async getBalanceSheetList(stockCode: string, dateType: string = '年报', englishKey: boolean = false): Promise<BalanceSheet[]> {
    const dateTypeMap: Record<string, string> = {
      '年报': '001',
      '中报': '002',
      '一季报': '003',
    ...
    if (englishKey) {
      return tableData.map(item => this.convertKeys(this.keyMappings.companyInfo, item) as BalanceSheet);
    }

    return tableData as BalanceSheet[];
  }

  /**
   * 获取现金流量表数据
   */
  async getCashFlowStatementList(stockCode: string, dateType: string = '年报', englishKey: boolean = false): Promise<CashFlowStatement[]> {
    const dateTypeMap: Record<string, string> = {
      '年报': '001',
      '中报': '002',
      '一季报': '003',
    ...
    if (englishKey) {
      return tableData.map(item => this.convertKeys(this.keyMappings.companyInfo, item) as CashFlowStatement);
    }

    return tableData as CashFlowStatement[];
  }

  /**
   * 获取财务分析数据
   */
  async getFinancialAnalysisList(stockCode: string, dateType: string = '年报', englishKey: boolean = false): Promise<FinancialAnalysis[]> {
    const dateTypeMap: Record<string, string> = {
      '年报': '001',
      '中报': '002',
      '一季报': '003',
    ...
    if (englishKey) {
      return tableData.map(item => this.convertKeys(this.keyMappings.companyInfo, item) as FinancialAnalysis);
    }

    return tableData as FinancialAnalysis[];
  }

  /**
   * 获取募资明细
   */
  async getStockFundList(stockCode: string, englishKey: boolean = false): Promise<FundInfo[]> {
   ...
   * 获取交易明细
   */
  async getStockTradeList(stockCode: string, englishKey: boolean = false): Promise<TradeInfo[]> {
    ...
    titleDivs.each((index, titleDiv) => {
      ...
      if (tradeMoneyText) {
        ...
        if (key && value) {
          (tradeInfo as any)[this.cleanText(key)] = this.cleanText(value);
        }
      }
   ...
   * 获取事件提醒
   */
  async getStockEventList(stockCode: string, englishKey: boolean = false): Promise<EventInfo[]> {
   ...
   * 获取公告列表
   */
  async getStockNoticeList(stockCode: string, page: number = 1): Promise<PaginatedResponse<NoticeInfo>> {
    ...
    try {
      ...
      const noticeInfos: NoticeInfo[] = lists.map((item: any) => ({
        id: item.id || '',
        title: item.title || '',
   ...
   * 获取定增计划
   */
  async getStockSurvey(stockCode: string, englishKey: boolean = false): Promise<SurveyInfo> {
    const url = `${this.baseUrl}/company-show/stock-${stockCode}-tab-survey.html#content`;
    const html = await this.request(url);
    const $ = this.parseHTML(html);

    const surveyInfo: any = {};
    ...
    return surveyInfo as SurveyInfo;
  }

  /**
   * 获取做市商信息
   */
  async getStockBrokerList(stockCode: string, englishKey: boolean = false): Promise<BrokerInfo[]> {
   ...
   * 获取质押信息
   */
  async getStockPledgeData(stockCode: string, englishKey: boolean = false): Promise<PledgeData> {
   ...
   * 获取研报列表
   */
  async getStockReportList(stockCode: string, englishKey: boolean = false): Promise<ReportInfo[]> {
   ...
   * 键名转换工具
   */
  private convertKeys(mapping: Record<string, string>, data: any): any {
    const converted: any = {};
