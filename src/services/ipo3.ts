...
import { StockInfo, CompanyInfo, StockQueryResult, DataSource, IPO3Response } from '../types/stock.js';
...
export class IPO3Service {
   ...
   */
  async getCompanyDetail(stockCode: string): Promise<CompanyInfo | null> {
    try {
   ...
   * 解析HTML页面数据
   */
  private parseCompanyHtml(html: string, stockCode: string): CompanyInfo | null {
    try {
      // 基础数据
   ...
   * 提取多个匹配的文本
   */
  private extractMultipleTexts(html: string, selector: string): string[] {
    try {
   ...
   * 解析公司数据
   */
  private parseCompanyData(data: IPO3Response, stockCode: string): CompanyInfo {
    return {
      name: data['股票名称'] as string || '',
      code: stockCode,
      price: data['最新价'] as string || '0',
      change: data['涨跌额'] as string || '0',
      changePercent: data['涨跌幅'] as string || '0%',
      industry: data['所属行业'] as string || '',
      open: data['今开'] as string || '0',
      high: data['最高'] as string || '0',
      avgPrice: data['平均价'] as string || '0',
      peRatio: data['市盈率'] as string || '',
      volume: data['成交量'] as string || '0',
      totalMarketValue: data['总市值'] as string || '',
      prevClose: data['昨收'] as string || '0',
      low: data['最低'] as string || '0',
      turnoverRate: data['换手率'] as string || '',
      pbRatio: data['市净率'] as string || '',
      amount: data['成交额'] as string || '0',
      circulatingMarketValue: data['流通市值'] as string || '',
      companyName: data['公司名称'] as string || '',
      companyWebsite: data['公司网址'] as string || '',
      companyPhone: data['公司电话'] as string || '',
      secretary: data['董秘'] as string || '',
      secretaryEmail: data['董秘Email'] as string || '',
      secretaryPhone: data['董秘电话'] as string || '',
      legalPerson: data['法人'] as string || '',
      sponsor: data['主办券商'] as string || '',
      tradingMethod: data['交易方式'] as string || '',
      listingDate: data['挂牌日期'] as string || '',
      establishmentDate: data['成立日期'] as string || '',
      marketMakingDate: data['做市日期'] as string || '',
      registeredCapital: data['注册资本'] as string || '',
      region: data['所属地区'] as string || '',
      officeAddress: data['办公地址'] as string || '',
      companyProfile: data['公司简介'] as string || '',
      mainBusiness: data['主营业务'] as string || '',
      businessScope: data['经营范围'] as string || '',
      financingStatus: data['融资状态'] as string || '',
      actualRaisedNetAmount: data['实际募资净额'] as string || '',
      financingSuccessRate: data['融资成功率'] as string || '',
      financingRanking: data['融资排名'] as string || '',
   ...
   * 解析股本信息
   */
  private parseEquityStructure(data: any): any[] {
   ...
   * 解析高管信息
   */
  private parseSeniorManagement(data: any): any[] {
    if (!Array.isArray(data)) return [];
    return data;
  }
   ...
   * 解析新闻资讯
   */
  private parseNews(data: any): any[] {
    if (!Array.isArray(data)) return [];
    return data;
  }
   ...
   */
  private convertToStockInfo(companyInfo: CompanyInfo): StockInfo {
    return {
   ...
   * 从搜索结果中提取股票代码
   */
  private extractStockCodesFromSearch(html: string): string[] {
    const codes: string[] = [];
   ...
   * 获取实时行情（单个股票）
   */
  async getRealtimeQuote(code: string): Promise<StockQueryResult> {
