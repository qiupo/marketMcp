 ...
 * 转换字段名（中英文映射）
 */
export function convertKeys(mapping, data) {
    const converted = {};
 ...
 * 提取表格数据
 */
export function extractTableData(document, selector) {
    ...
    if (rows.length === 0)
        return [];
    // 提取表头
    const headers = Array.from(rows[0].querySelectorAll('th, td')).map(th => cleanText(th.textContent || ''));
    // 提取数据行
 ...
 * 提取财务表格数据（用于财务报表解析）
 */
export function extractFinanceTable(document) {
    const financeTable = document.querySelector('.finance-tab');
    if (!financeTable)
        return [];
    const headers = Array.from(financeTable.querySelectorAll('tr th')).map(th => cleanText(th.textContent || ''));
    if (headers.length === 0)
        return [];
 ...
 * 解析股本结构数据
 */
export function parseEquityStructure(mainElement) {
    if (!mainElement)
        return [];
 ...
 * 解析高管介绍数据
 */
export function parseSeniorManagement(mainElement) {
    if (!mainElement)
        return [];
    const headers = Array.from(mainElement.querySelectorAll('thead th')).map(th => cleanText(th.textContent || ''));
    const management = [];
 ...
 * 解析新闻列表数据
 */
export function parseNewsList(mainElement) {
    if (!mainElement)
        return [];
    const newsItems = [];
 ...
 * 解析股东结构数据
 */
export function parseShareholderStructure(mainElement) {
    if (!mainElement)
        return [];
    const tableData = extractTableData(mainElement, 'table');
    return tableData.map(item => ({
 ...
 * 解析投资者信息
 */
export function parseInvestorInfo(mainElement) {
    if (!mainElement)
        return [];
    const tableData = extractTableData(mainElement, 'table');
    return tableData.map(item => ({
...
export class IPO3Error extends Error {
    code;
    stockCode;
    constructor(message, code, stockCode) {
        super(message);
        this.code = code;
